...
__global__ void Conv_Kernel(float *d_in_img, float *d_layer_wt,
    ...
    if (row < num_rows && col < num_cols){
        ...
        float prod = 0.0f;
        int img_index, img_index_out, row_val, col_val, filter_index, num_rows_img, num_cols_img, filter_jump, ch_index;
        ...
        for (int i=0;i<num_filters;i++){
            ...
            prod = d_layer_bias[i];
            ch_index = (i*num_channels*kernel_size*kernel_size);
                ...
                for (int k=0;k<kernel_size;k++){
                    ...
                    for (int l=0;l<kernel_size;l++){
                        ...
                        if (row_val >= 0 && row_val < num_rows_img && col_val >= 0 && col_val < num_cols_img){
                            ...
                            filter_index = (k*kernel_size) + l + (j*kernel_size*kernel_size) + ch_index;
